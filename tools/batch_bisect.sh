#!/bin/bash
# batch_bisect.sh — Find optimal batch-size/ubatch-size via bisection + saturation test
#
# Usage:
...
if [ "$TEST_BATCH" -gt 0 ] 2>/dev/null; then
  ...
  # ── PHASE 1: COARSE SWEEP (bracket lo/hi) ──
  log ""; log "=== PHASE 1: COARSE SWEEP ==="
  LO=0; HI=0; BATCH=4096
  log "  Testing batch=$BATCH..."
  set_batch "$BATCH"; restart; log "  Tiny probe..."
  if tiny_probe; then
    # 4096 PASS → sweep up (double) until OOM
    log "  PASS at 4096 — sweeping up"
    LO=4096; BATCH=8192
    while true; do
      log ""; log "  Testing batch=$BATCH..."
      set_batch "$BATCH"; restart; log "  Tiny probe..."
      if tiny_probe; then
        log "  PASS"; LO=$BATCH; BATCH=$((BATCH * 2))
      else
        log "  OOM"; HI=$BATCH; break
      fi
      [ "$BATCH" -gt 131072 ] && { log "  Stopping sweep at $BATCH"; HI=$BATCH; break; }
    done
  else
    # 4096 OOM → sweep down (halve) until first PASS
    log "  OOM at 4096 — sweeping down"
    HI=4096; BATCH=2048
    while true; do
      log ""; log "  Testing batch=$BATCH..."
      set_batch "$BATCH"; restart; log "  Tiny probe..."
      if tiny_probe; then
        log "  PASS"; LO=$BATCH; break
      else
        log "  OOM"; HI=$BATCH; BATCH=$((BATCH / 2))
      fi
      if [ "$BATCH" -lt 64 ]; then
  ...
  log ""; log "  Bracket: lo=$LO (PASS), hi=$HI (OOM)"
  fi
