...
6. **OOM** → step down and repeat. `batch_bisect.sh` Phase 1 now **starts at ctx** (the hard ceiling — the max useful batch can never exceed ctx, since prefill is bounded by ctx and decode is 1 token per stream) and **halves on failure** (`ctx → ctx/2 → ctx/4`) until a batch passes probe + saturation:
...
9. **Phase 2 — TRUE test: full-context saturation + compaction.** The tiny probe only proves the compute graph fits at near-empty context. `-fit` leaves ~zero headroom, so memory peaks near full context (KV compaction temp buffers, hybrid/SSM state re-derivation) — a value can still OOM once the context fills and compacts. Note the prefill (PP) graph buffer scales ~2 MiB/unit and is **far larger than the tiny probe's decode graph**, so a value that passes the tiny probe can still OOM on a long-prompt prefill — the saturation run re-brackets the winner downward (observed on both the 35B and 9B models).
   ...
   - **Performance sweep (Phase 4)** — after the ceiling is found, `batch_bisect.sh` sweeps `{ceiling, /2, /4, /8, /16, 2048}` below it and scores each with combined throughput over a representative workload (75%-ctx prefill + 4000-token decode), which self-weights by phase duration (large-ctx models → prefill matters; small-ctx → decode). **GPU-residency filter**: any candidate whose decode averages >200% CPU (compute spilled to CPU) is EXCLUDED — the documented rationale is that a batch right before CPU involvement is inherently slower (e.g. Gemma 12B Q6: 36 t/s with draft on CPU vs 59.8 on GPU). Top-2 finalists are re-run 3×; the fastest **100%-GPU-resident** batch is written to models.ini. A CPU-compute model (e.g. gpt-oss, ~900-1100% CPU) will have all candidates spill → the sweep reports the ceiling instead.
   - **Residency-gated Phase 4 skip** — when the ceiling IS ctx (small-ctx model that fits at ctx on the first probe), a single residency check runs before the sweep: GPU-resident ctx (avg CPU ≤ 200%) → **Phase 4 is skipped**, ctx is written to models.ini (the sweep would only return ctx anyway). CPU-spilled ctx (avg CPU > 200%) → Phase 4 runs and its residency filter finds the largest GPU-resident batch below (e.g. a 128K ctx that passes OOM but decodes on CPU while 32K is 100% GPU → 32K wins). Only the fits-at-ctx case can skip; large-ctx models where ctx OOMs (ceiling < ctx) always run Phase 4.
...
- The `[*]` preset default `batch-size`/`ubatch-size = 4096` is a deliberate **fallback for untuned entries** — it is NOT a tuning starting point anymore (Phase 1 starts at ctx). It OOMs immediately on most untuned entries, which is the signal to run `batch_bisect.sh` and override per `[model]`. Never run an untuned entry at 4096.
