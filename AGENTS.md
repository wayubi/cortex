...
1. `CAP = min(ctx-size, 16384)`. Walk one **coarse ladder** of powers of two from 256 up to CAP, stopping early once two consecutive rungs measure below the running best prefill × `PREFILL_TOL` (3%). Only a ceiling-edge midpoint refinement (max 2 extra restarts, e.g. the small-ceiling MoE class where 448 sits between 256 and 512) ever tests a non-power-of-two.
...
3. **Pick = the smallest PASS batch whose prefill is within `PREFILL_TOL` (3%) of the best prefill.** A smaller batch leaves more VRAM headroom for the MTP draft buffer and for larger-ctx siblings, at no measured prefill cost.
4. **Confirm at pick, one restart**: `saturation_test` (~99% ctx, with compaction or ceiling truncation) → `long_decode_check` (real prompt, sustained decode) → on GPU models a `decode_sample` compared against the batch-256 baseline (re-sample once below `DECODE_CLIFF` 0.70× baseline; step down on a CPU placement signature; the 0.90–0.70 band is a WARN, never a failure). Max 2 step-downs to the next lower PASS rung, then fail with the ini restored.
