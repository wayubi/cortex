...
6. **OOM** → step down and repeat. `bench.sh bisect` Phase 1 now **starts at ctx** (the hard ceiling — the max useful batch can never exceed ctx, since prefill is bounded by ctx and decode is 1 token per stream) and **halves on failure** (`ctx → ctx/2 → ctx/4`) until a batch passes probe + saturation:
   - **Small-ctx models (4k–32k):** ctx itself usually passes on the **first probe** → the batch value is simply `ctx`. No VRAM-fit extrapolation, no batches above ctx are ever tested (this is what replaced the old 4096-start VRAM-math estimate, which chased valueless 262K/354K batches on 4k/8k models).
   - **Large-ctx models (64k–256k):** ctx OOMs fast (clean signal), then halving narrows to the VRAM-limited ceiling; the last OOM seen becomes `hi`, so Phase 2 bisects up to the true max. E.g. ornith 256k: 262144→…→1024 OOM, 512 PASS → bisect refines to ~960.
...
9. **Phase 2 — TRUE test: full-context saturation + compaction.** The tiny probe only proves the compute graph fits at near-empty context. `-fit` leaves ~zero headroom, so memory peaks near full context (KV compaction temp buffers, hybrid/SSM state re-derivation) — a value can still OOM once the context fills and compacts. Note the prefill (PP) graph buffer scales ~2 MiB/unit and is **far larger than the tiny probe's decode graph**, so a value that passes the tiny probe can still OOM on a long-prompt prefill — the saturation run re-brackets the winner downward (observed on both the 35B and 9B models).
   ...
   - **The filler string MUST be long enough** — `bench.sh` builds the saturation/prefill prompt as `('The history of computing is long and complex. '*N)[:SIZE]`. A fixed `*1000` (46K chars ≈ 10.5K tokens) **silently under-saturates at ≥16K ctx** (the observed `prompt_tokens=9068` on a 64K model whose target was 55K — the gate then validates against a ~9K prefill and false-PASSes). The code now uses `*30000` (~314K tokens ≈ 1.3MB, still under nginx `8m`); if you hand-roll a saturation prompt keep N large enough for the ctx. Verify with `usage.prompt_tokens` ≈ 85% of ctx, not the filler length.
   - Send one request with a prompt filling ~90% of that ctx and `max_tokens` large enough to overflow it and force KV compaction (~20% of ctx):
     ...
     ```
   - **TRUE PASS** = no OOM markers **AND** `n_past` reaches the ctx limit **AND** the request completes HTTP 200 with the full generation. On pure-attention models a compaction/sequence-removal event appears in the log; on **hybrid/SSM models** (e.g. Qwen3.6) the server instead truncates at the ceiling (`truncated = 1`, `finish_reason = length`) because SSM state can't be KV-shifted — full-context saturation is still reached and that is the point of the test. (You cannot force `ctx-shift` here: the router rejects unknown preset keys with `option 'ctx-shift' not recognized in preset`.) The saturation gate allows an **EPSILON=8 token** tolerance near the ctx ceiling to avoid false SIZING failures on small-ctx models (e.g. gemma-4-12b @ 16k) whose decode stops 0-4 tokens short of ctx due to token-boundary noise.
   - **FAIL** = any OOM marker during prefill/decode/compaction, or a crash before compaction completes. The bisect is **saturation-gated**: a candidate only counts as PASS when it passes BOTH the tiny probe AND the saturation test, so the bisect converges directly to the saturation-safe ceiling (no step-down-by-64 grind). If the final confirm fails, a bracketed halve-down + bisect search finds the ceiling in O(log) runs. Core dumps are disabled for the llama-cpp container (`ulimits.core = 0`) so OOM crashes don't write multi-GB dumps.
   - **Performance sweep (Phase 4)** — after the ceiling is found, `bench.sh bisect` sweeps `{ceiling, /2, /4, /8, /16, 2048}` below it and scores each with combined throughput over a representative workload (75%-ctx prefill + 4000-token decode), which self-weights by phase duration (large-ctx models → prefill matters; small-ctx → decode). **GPU-residency filter**: any candidate whose decode pegs all cores to CPU (avg CPU > 200%) is EXCLUDED — the documented rationale is that a batch right before CPU involvement is inherently slower (e.g. Gemma 12B Q6: 36 t/s with draft on CPU vs 59.8 on GPU). 100-200% is noise and stays eligible. Top-2 finalists are re-run 3×; the fastest **100%-GPU-resident** batch is written to models.ini. A CPU-compute model (e.g. gpt-oss, ~900-1100% CPU) will have all candidates spill → the sweep reports the ceiling instead.
   - **Residency-gated Phase 4 skip** — when the ceiling IS ctx (small-ctx model that fits at ctx on the first probe), a single residency check runs before the sweep: GPU-resident ctx (avg CPU < 100%) → **Phase 4 is skipped**, ctx is written to models.ini (the sweep would only return ctx anyway). CPU-spilled ctx (avg CPU > 200%, all cores pegged) → Phase 4 runs and its residency filter finds the largest GPU-resident batch below (e.g. a 128K ctx that passes OOM but decodes on CPU while 32K is 100% GPU → 32K wins). 100-200% is AMBIGUOUS — treated as "not proven GPU" in the gate (investigates further), but eligible in the sweep. Only the fits-at-ctx case can skip; large-ctx models where ctx OOMs (ceiling < ctx) always run Phase 4.
   - **Long-decode check** — the saturation prompt uses repetitive filler, which yields high draft acceptance and can MASK a decode-time OOM (low acceptance → more target recomputes → the flash-attn VMM pool grows until `CUDA error: out of memory ... cuMemCreate`). Observed: `qwen-3.5-9b-q4-mtp-16k-think` passed full-context saturation at batch 2048 but OOM'd at `n_decoded ≈ 5.4K` on a real long generation. After the filler saturation passes, also run a **real-prompt, sustained-generation test** (`max_tokens` ~6000, `ignore_eos`) to confirm the value survives long decode:
   ...
   - Saturation runs are ~2 min at 16K, growing roughly linearly with ctx — only run them on values that pass the quick filter. For 128K+ ctx, the body nears nginx's `client_max_body_size 8m` (set on the llama-cpp server block) and the `proxy_read_timeout 3600s` (raised from 600s after a 256K saturation run timed out at 600s); raise further or use a tighter prompt if the request returns 413/504. Note a client 504 does NOT cancel the llama-cpp generation — it keeps decoding server-side until the ctx ceiling or max_tokens, so check the logs for the true outcome.
...
- The `[*]` preset default `batch-size`/`ubatch-size = 4096` is a deliberate **fallback for untuned entries** — it is NOT a tuning starting point anymore (Phase 1 starts at ctx). It OOMs immediately on most untuned entries, which is the signal to run `bench.sh bisect` and override per `[model]`. Never run an untuned entry at 4096.
...
1. **n_max sweep {2,3,4,5}** at p_min=0.7 (speed axis, 4 runs) — picks the fastest clean n_max; winner is then re-confirmed once with the strict <0.05 gate at the sweep p_min (1 run) — on failure, the model is failed (exit non-zero).
...
**Total: 11 decode runs (~13-16 min).** Each model's p_min is empirically discovered; the sweep is the source of truth (not `models.ini`, not a global constant).
